return {
    ...
    --
    {
        ...
        config = function()
                ...
                actions = {
                    files = {
                        ...
                        ['ctrl-v'] = function(selected, opts)
                            local file = require('fzf-lua').path.entry_to_file(
                                selected[1],
                                opts
                            )
                            local cmd =
                                string.format(':rightbelow vnew %s', file.path)
                            vim.cmd(cmd)
                        end,
            ...
            keymapFn(
                '<leader>sb',
                "lua require('fzf-lua').blines({ winopts = { preview = { horizontal = 'right:60%' }}})",
            ...
            keymapFn(
                '<leader>sW',
                require('fzf-lua').grep_cWORD,
                { desc = '[S]earch current uppercase [W]ord' }
            )
            keymapFn(
                '<leader>sw',
                require('fzf-lua').grep_visual,
                { mode = 'v', desc = '[S]earch current visual [Word]' }
            )
            ...
            keymapFn(
                '<leader>gc',
                "lua require('fzf-lua').git_commits({ winopts = { height = 1, width = 1 }})",
                { desc = '[G]it [C]ommits' }
            )
            keymapFn(
                '<leader>bc',
                "lua require('fzf-lua').git_bcommits({ winopts = { height = 1, width = 1 }})",
            ...
            keymapFn(
                ...
                { desc = '[X] Diagnostics [L]ist' }
            )
            keymapFn(
                '<leader>xd',
                require('fzf-lua').diagnostics_document,
                { desc = '[X] Diagnostics [D]ocument' }
            )
            keymapFn(
                '<leader>xw',
                require('fzf-lua').diagnostics_workspace,
                { desc = '[X] Diagnostics [W]orkspace' }
            )
        end,
    },
